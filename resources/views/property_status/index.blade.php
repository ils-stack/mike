    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="mb-2">
            ...
            <input type="text" id="status_name" class="form-control" autocomplete="off">
        </div>

        <div class="mb-2">
            <label class="form-label">Marker Letter</label>
            <input type="text"
                   id="marker_letter"
                   maxlength="2"
                   class="form-control text-uppercase"
                   placeholder="N" autocomplete="off">
        </div>
        ...
        data.forEach((row, i) => {
            rows += `
              <tr>
                <td>${i + 1}</td>
                <td>${row.status}</td>
                <td class="text-center fw-bold">${row.marker_letter || ''}</td>
                <td class="text-center">
                  <span style="
                    ...
                    height:22px;
                    border-radius:50%;
                    background:${row.marker_color || '#000'};
                ...
                <td>
                  <i class="fa fa-edit text-primary me-2 edit-status"
                     ...
                     data-letter="${row.marker_letter || ''}"
                     data-color="${row.marker_color || '#1E88E5'}"></i>
...
function openStatusModal() {
    ...
    $('#status_name').val('');
    $('#marker_letter').val('');
    $('#marker_color').val('#1E88E5'); // default blue
...
$(document).on('click', '.edit-status', function () {
    ...
    $('#marker_letter').val($(this).data('letter'));
    $('#marker_color').val($(this).data('color'));
...
function saveStatus() {
    ...
    let data = {
        ...
        marker_letter: $('#marker_letter').val(),
