...
@for($k=0;$k<$budget_cnt;$k++)
    ...
    foreach($user_fields as $e_key => $e_val){
      foreach($e_val as $sub_key => $sub_val){
        if($sub_val == $field){
          ...
          $client_total = $field_data;
          break;
        }
      }
    }
    ?>
    <td><input type = "text" class = "form-control" name = "{{$field}}" id = "{{$field}}" value = "{{$assetObj->formatRand($field_data)}}"></td>
  @endfor
  ...
  @for($j=0;$j<$last_col_sp;$j++)
    ...
    ?>
    <td><input type = "text" class = "form-control" name = "{{$sp_field}}" id = "{{$sp_field}}" value = "{{$assetObj->formatRand($field_data)}}"></td>
  @endfor
...
@endfor
<tr class="total-row">
  <td colspan="5">Total records: <span class="highlight">{{$budget_cnt}}</span></td>
</tr>
